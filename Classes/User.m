...
@synthesize pk, serverpk, userName, password, udid, service, playerName, contactID, fbUID, latestHI;
...
- (void)fromDB {
    ...
    if (success == SQLITE_ROW) {
		...
		str = (char *)sqlite3_column_text(hydrate_statement, 6);
        self.playerName = (str) ? [NSString stringWithUTF8String:str] : nil;
        self.contactID = sqlite3_column_int(hydrate_statement, 7);
        self.fbUID = sqlite3_column_int(hydrate_statement, 8);
        self.latestHI = sqlite3_column_double(hydrate_statement, 9);
   } else {
        self.pk = 0;
        self.serverpk = 0;
        self.userName = nil;
...
- (void)toDB {
	...
	if (dehydrate_statement == nil) {
		char *sql = "UPDATE user SET serverpk=?, username=?, password=? , udid=?, service=?, playerName=? , contactID=?, fbUID=?, latestHI=? where pk=?";
	...
	sqlite3_bind_text(dehydrate_statement, 6, [playerName UTF8String], -1, SQLITE_TRANSIENT);
	sqlite3_bind_int(dehydrate_statement, 7, self.contactID);
	sqlite3_bind_int(dehydrate_statement, 8, self.fbUID);
	sqlite3_bind_double(dehydrate_statement, 9, self.latestHI);
	sqlite3_bind_int(dehydrate_statement, 10, pk);
	// Execute the query.
...
-(void)updateService
{
	if([[Reachability sharedReachability] internetConnectionStatus] == NotReachable){
		...
		NSString *str=[NSString stringWithFormat:@"http://www.golfmemoir.com/unlock_download.php?udid=%@", [[UIDevice currentDevice] identifierForVendor]];
...
-(void)uploadService
{
	if([[Reachability sharedReachability] internetConnectionStatus] == NotReachable){
		...
		NSString *str=[NSString stringWithFormat:@"http://www.golfmemoir.com/user_updateService.php?udid=%@&service=%i", [[UIDevice currentDevice] identifierForVendor], service];
